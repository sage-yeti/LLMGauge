...
The current seed catalog contains 16 model variants and 21 GPU profiles. It is a reviewed starting set, not an exhaustive or automatically refreshed inventory. The recent expansion adds Qwen3 4B/8B, Qwen2.5-Coder 7B, Llama 3.1 8B, Gemma 3 12B/27B, Phi-4 Mini, and Mistral Nemo 12B; GPU additions cover older and newer NVIDIA cards, AMD RDNA 2/3/4, Intel Arc Battlemage, and AMD Radeon 780M integrated graphics. The choice was guided by distinct size/use cases and documented memory classes rather than catalog volume.

Model identity, publisher, license, parameter class, and maximum context values link to publisher model cards. For new entries, a publisher-defined default context is not consistently available, so it is omitted and the engine reports practical context guidance as unavailable rather than assuming one. GGUF availability is separately attributed to the relevant GGUF repository; publisher-converted and third-party files are identified as conversions, not official publisher weights. The available Q4_K_M and Q8_0 candidates are approximate planning inputs: model weights are calculated from documented/rounded parameter count and the quantization's bits-per-weight, not copied from a particular download's byte size. Actual GGUF files can differ due to tensor layout, metadata, tokenizer, and conversion choices. GPU memory capacity/type and architecture are sourced from manufacturer specifications; integrated graphics are recorded with zero dedicated VRAM when no fixed dedicated allocation exists. Their shared system memory is deliberately left unspecified.
